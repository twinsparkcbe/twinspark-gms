...
const RECORD_COLUMNS =
  "id, employee_id, attendance_date, status, check_in, check_out, working_minutes, daily_wage, payable_amount";
...
export async function saveDailyAttendance(
  ...
  // The wage snapshot (0033). Two lookups, because the rule differs by case:
  //  - a record being created takes the employee's CURRENT rate;
  //  - a record being edited KEEPS the rate it was first saved with.
  // Without the second, correcting a typo in August's attendance after an
  // October raise would silently reprice August.
  const employeeIds = input.entries.map((entry) => entry.employeeId);

  const [{ data: employees, error: employeeError }, { data: existing, error: existingError }] = await Promise.all([
    supabase.from("attendance_employees").select("id, daily_wage").in("id", employeeIds),
    supabase
      .from("attendance_records")
      .select("employee_id, daily_wage")
      .eq("attendance_date", input.attendanceDate)
      .in("employee_id", employeeIds),
  ]);

  if (employeeError) throw new Error(toErrorMessage(employeeError, "Failed to save attendance."));
  if (existingError) throw new Error(toErrorMessage(existingError, "Failed to save attendance."));

  const currentWage = new Map((employees ?? []).map((e) => [e.id, e.daily_wage]));
  const frozenWage = new Map(
    (existing ?? []).filter((r) => r.daily_wage !== null).map((r) => [r.employee_id, r.daily_wage])
  );
  ...
  const payload = input.entries.map((entry) => ({
    ...
    // An already-priced record keeps its rate; anything else takes today's.
    daily_wage: frozenWage.get(entry.employeeId) ?? currentWage.get(entry.employeeId) ?? null,
    // working_minutes and payable_amount deliberately absent — generated.
  }));
