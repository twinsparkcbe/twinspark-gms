...
async function loadRequireCustomersAccess() {
  const mod = await import("./require-customers-access");
  return mod.requireCustomersAccess;
}

describe("requireCustomersAccess", () => {
  beforeEach(() => {
  ...
  it("redirects to /login when there is no session", async () => {
    ...
    const requireCustomersAccess = await loadRequireCustomersAccess();

    await expect(requireCustomersAccess()).rejects.toThrow("REDIRECT:/login");
  ...
  it("returns the user when they are an active Sales Person, without redirecting", async () => {
    ...
    const result = await requireCustomersAccess();
...
describe("requireCustomersAccess — Mechanic", () => {
  beforeEach(() => {
    redirectMock.mockClear();
    getSessionAccessMock.mockReset();
    signOutMock.mockClear();
  ...
  it("allows an active Mechanic", async () => {
    ...
    const guard = await loadRequireCustomersAccess();
